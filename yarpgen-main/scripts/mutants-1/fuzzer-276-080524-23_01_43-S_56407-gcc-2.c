...
{
  int  YNeL4PGX = 2147483647;
  int * ES4Qjtal = &YNeL4PGX;
  unsigned char HFzYOhn3 = (unsigned char)0;
  int j;
  int n;
  int v[64];
  int w[64];
  ...
  transparent_crc(HFzYOhn3, "HFzYOhn3", 1);
  platform_main_end(crc32_context ^ 0xFFFFFFFFUL, 0);
  exit(0);
}
...
int main(void)
{
  platform_main_begin();
  crc32_gentab();
  int  AHpxV2fJ = -1716696614;
  int * sywnvoSP = &AHpxV2fJ;
  test(2, 1);
  if (!sywnvoSP)
    __builtin_abort();
  return 0;
