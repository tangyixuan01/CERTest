...
def get_blacklist():
    ...
    #choice = ['for', 'if', 'while', 'assignment', 'switch']
    choice = ['switch']
    for snippet_type in choice:
        ...
        for file_name in snippet_file_paths:
            ...
            if os.path.isfile(snippet_file_path):
                        ...
                        if first_line.startswith('//'):
                            ...
                            if first_line.strip() != '':
                                for variable_type in first_line.split(';'):
                                    ...
                                    snippet_variable2type[variable] = type_decl
                shuzu = False
                for eachtype in snippet_variable2type.values():
                    if eachtype.find('[') != -1 or eachtype.find('struct') != -1 or eachtype.find('union') != -1 or eachtype.find('func') != -1:
                        #数组
                        shuzu = True
                        break
                if shuzu:
                    print(',,,,,,,,,,,,,,,shuzu or struct')
                    shutil.move(snippet_file_path,'/home/yixuan/compiler_testing-main-new/scripts/util/mutate/codesnippet/switch-unsuccess/')
                    continue
                init_stmts, ori_vari2new_vari, new_vari2type_decl = construct_init_stmts_4_snippet(global_decl, snippet_variable2type)
                ...
                #include <random>', '#include <stdarg.h>', '#include <tuple>', '#include <mmintrin.h>', '#include <iostream>', '#include <stdio.h>', ' #include <string>', '#include <array>', '#include <unistd.h>', '#include <signal.h>', '#include <algorithm>', '#include <vector>
                main_code = '#include <stdio.h>\n'+'#include <stdlib.h>\n'+'#include <math.h>\n' +'#include <stdarg.h>\n'+'#include <string.h>\n'+'#include <unistd.h>\n'+'#include <signal.h>\n'+ code_generator.visit(main_node)
                #main_code = 'typedef struct __builtin_va_list { } __builtin_va_list;\n'+ code_generator.visit(main_node)
                cnt += 1
                ...
                compile_program_cmd = ['/home/yixuan/compiler/pre-build-llvm/clang+llvm-12.0.0-x86_64-linux-gnu-ubuntu-20.04/bin/clang', 'tmp.c', '', '-w']
                ...
                print('ret_code is: '+str(ret_code))
                if ret_code != 0:
                    ...
                    shutil.move(snippet_file_path,'/home/yixuan/compiler_testing-main-new/scripts/util/mutate/codesnippet/switch-success/')
...
if __name__ == '__main__':
    ...
    exit()
    selects = ['assignment', 'for', 'if', 'while', 'switch']
