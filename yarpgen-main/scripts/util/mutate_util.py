...
@LastEditTime : 2023-12-10 21:57:25
@FilePath     : mutate_util.py
@Description  : 程序变异
@
...
def append_hash_4_local_variable(global_decl, root):
    """
    @description: 对局部变量进行hash，将hash后的语句插入到nodes中
        不进行hash的变量：1. 指针类型 2. 未初始化的变量
    ...
    if isinstance(nodes, list) and\
        ...
        # 如果最后一个结点是return语句，则将hash语句插入到return语句之前
...
def call_func_in_main(global_decl, main_node):
    """
    @description: 在main函数中调用其他函数
    @param {*} global_decl 全局变量声明
    @param {*} main_node main函数结点
    @return {*}
    """
...
def random_mutate_program(global_decl, ast):
    """
    @description: 随机选择一个变异算子, 在ast层面进行变异
    @param {*} global_decl 全局变量信息
    @param {*} ast 待变异的ast
    @return {*}
    """
    ...
    while True:
        ...
        opts = ['insert-for', 'insert-if', 'insert-while', 'insert-assignment', 'insert-switch']
        opt = random.choice(opts)
        print('mutation selection : '+opt)
        opt = opt.split('-')
        if opt[0] == 'insert':
            insert_code_snippet_node(global_decl, ast, opt[1])
            mutation_operator_label = 'insert-{}-snippet'.format(opt[1])
            print('insert-{}-snippet: '+str(mutation_operator_label))
            break
        elif opt[0] == 'prune':
...
def mutate_program_with_selected_mutator(global_decl, ast, selected_mutator):
    """
    @description: 使用选择的变异算子变异程序
    @param {*} global_decl 全局变量信息
    @param {*} ast 待变异的ast
    @param {*} selected_mutator 选择的变异算子
    @return {*} 变异算子标签
    """
...
def mutate_program_csmith(test_dir, proc_num=-1, seed=-1, lock=None):
    ...
    # ! 测试时解除下面注释
    # return mutation_operator_labels

    # mutation_operator_label = 'none'
...
def mutate_program_yarpgen(test_dir, proc_num=-1, seed=-1, lock=None):
    # ! 测试时解除下面注释
    # exec_cmd('../../yarpgen -s 1005070510 --std=c -o ' + test_dir)
...
def random_fuzzer_one2one(seed_programs_dir, target_programs_dir):
    """
    @description: 程序随机变异多次，针对每个程序生成一个程序变体(one2one)
    @param {*} seed_programs_dir 种子程序存放目录
    @param {*} target_programs_dir 程序变体存放目录
    @return {*}
    """
...
def save_csmith_program_file(target_file_path, csmith_program_ast, mutation_operator_labels='none'):
    """
    @description: 将csmith程序保存
    @param {*} target_file_path 保存地址
    @param {*} csmith_program_ast 待保存的AST
    @param {*} mutation_operator_labels 变异历程, 例如： insert-while-snippet -> prune-if-snippet
    @return {*}
    """
    ...
    with open(target_file_path, 'w') as f:
        ...
        lines.insert(0, '#include "/home/yixuan/csmith/runtime/csmith.h"\n')
        #new_code = '\n'.join(lines)
        new_code1 = []
        for each in lines:
            #print(each)
            ...
            if each.startswith('type'):
                ...
                new_code1.append(each)
        flag = True
        for eachline in new_code1:
            if 'platform_main_begin()' in eachline:
                flag  = False
                break
        if flag:
            new_code2 = []
            flagmain = False
            for index in range(len(new_code1)):
                if 'main(' in new_code1[index]:
                    new_code2.append(new_code1[index])
                    new_code2.append(new_code1[index+1])
                    new_code2.append('  platform_main_begin();')
                    new_code2.append('  crc32_gentab();')
                    flagmain = True
                else:
                    if flagmain:
                        flagmain = False
            ...
            new_code3 = []
            for index in range(len(new_code2)):
                if 'transparent_crc' in new_code2[index] and 'transparent_crc' not in new_code2[index+1]:
                    new_code3.append(new_code2[index])
                    new_code3.append('  platform_main_end(crc32_context ^ 0xFFFFFFFFUL, 0);')
                else:
                    new_code3.append(new_code2[index])
...
def random_fuzzer(seed_programs_dir, target_programs_dir, 
                  ...
                  lasting_time=20):
    """
    @description: 程序随机变异多次并保存
    @param {*} seed_programs_dir 种子程序文件夹
    @param {*} target_programs_dir 保存变异程序的文件夹
    @param {*} tmp_dir 执行未定义行为检测的临时文件夹
    @param {*} mutate_num 程序变异最高次数
    @param {*} is_need_check 是否需要未定义行为检测
    @param {*} lasting_time 持续时间，单位为秒s
    @return {*}
    """
    # 文件夹不存在则创建
    if not os.path.exists(target_programs_dir):
    ...
    while True:
        for test_program in seed_program_paths:
            remove_file('/home/yixuan/codeExtract/mutants/','/home/yixuan/codeExtract/mutants-old/')
            print('/////////////begin a new program///////////////')
            print(test_program)
            name = test_program.split('/')[-1][:test_program.split('/')[-1].index('.')]
            global program_file_path
            program_file_path.append(test_program)
            # 初始化种子程序
            '''
            ...
            #print('11111111111')
            global_decl, main_node = get_global_decl_and_main_node(ast)
            print('get global_decl, main_node')
            #print(str(global_decl))
            #print(str(main_node))
            #print('---------------')
            #exit()
            global_decl.init()
            ...
            # 变异并保存
            mutation_operator_labels = ''
            #shutil.move(test_program,'/home/yixuan/codeExtract/gcc-valid-explore/')
            print('already to remove')
            for i in range(mutate_num):
                mutation_operator_label = random_mutate_program(global_decl, ast)
                if i == 0:
                    print('this is the i-th mutation: '+str(i))
                    mutation_operator_labels += mutation_operator_label
                else:
                    print('this is the i-th mutation: '+str(i))
                    mutation_operator_labels += ' -> ' + mutation_operator_label
    ...
    while True:
        for test_program in seed_program_paths:
            ...
            for i in range(mutate_num):
                ...
                # 更新reward信息
                mutator_scheduler.calc_reward_1(mutator, is_free)
...
def random_fuzzer_for_experiment():
    #random_fuzzer('/media/jwzeng/zjw_SSD/workplace/experiment/generator/csmith', 
    #              '/media/jwzeng/zjw_SSD/workplace/experiment/generator/my-mutator/mutants', 
    #             '/media/jwzeng/zjw_SSD/workplace/experiment/generator/my-mutator/tmp', 
    #              7, True, 86400)
    random_fuzzer('/home/yixuan/codeExtract/gcc-valid1', 
                  '/home/yixuan/codeExtract/mutants', 
                  '/home/yixuan/codeExtract/tmp', 
                  20, True, 86400)
    

def diversity_guidance_fuzzer_for_experiment():
    diversity_guidance_fuzzer('/media/jwzeng/zjw_SSD/workplace/experiment/generator/csmith', 
                              '/media/jwzeng/zjw_SSD/workplace/experiment/generator/my-mutator/03-diversity-guidance-mutants-24h-rewardis1-v3', 
                              '/media/jwzeng/zjw_SSD/workplace/experiment/generator/my-mutator/03-diversity-guidance-tmp-24h-rewardis1-v3', 
                              7, True, 60 * 60 * 24)


if __name__ == "__main__":
    # program_mutate_and_save('/home/jwzeng/workplace/dataset/experiment/csmith', '/home/jwzeng/workplace/dataset/experiment/csmith-mutate-1')
    ...
    #     compile_program_cmd = ['clang-18', 'test.c', '-I/home/jwzeng/tools/csmith/include', '-w']
    ...
    while True:
        ...
        print(labels)
        compile_program_cmd = "icx -I/home/jwzeng/tools/csmith/include -std=c99 -fPIC -mcmodel=large -w -fopenmp-simd -mllvm -vec-threshold=0 -O3 -o icx_opt_o3_test.o -c test.c".split()
