...
"""
@Author       : jun-wei-zeng
@Date         : 2023-11-05 19:03:54
@LastEditTime : 2023-12-09 22:49:39
@FilePath     : mutator_scheduler.py
@Description  : 
@
...
def calc_jaccard_similarity(code1, code2):
    ...
    # 并集的元素个数减去两个集合的交集的元素个数 = 两个集合中各自独有的元素的集合（也称为对称差）
...
class MutatorScheduler:
    ...
    def __init__(self, total_code=[], total_cfg=[]):
        ...
        self.total_cnt = 0                  # 总的变异次数
        self.similarity_sum = 0             # 相似度总和
    ...
    def get_mutator(self):
        ...
        for mutator, cnt_and_reward in self.mutator2cnt_and_reward.items():
            t = self.total_cnt              # 总变异次数
            tj = cnt_and_reward[0]          # tj表示直到第t次迭代为止选择第j个增变器/限制选项的总数
            xji_sum = cnt_and_reward[1]     # xji指第i次迭代中第j个增变器/限制选项的奖励；这里是tj次总和
            # UCB-1
            if tj != 0 and t > 1:
    ...
    def calc_reward_1(self, mutate_label, is_free, compiler='/home/jwzeng/compilers/gcc/gcc-13.1.0/bin/gcc', proc_num=-1):
        ...
        if ret_code == 0:
            ...
            # 如果此次变异相似度大于之前所有变异的相似度平均值，且该变异不会导致未定义行为，则奖励为1，否则奖励为0
            if sub_avg_of_similarity > avg_of_similarity and is_free:
                reward = 1
            else:
                reward = 0
            # 更新 变异算子 -> 具体变异算子和对应的奖励值和
    ...
    def calc_reward_2(self, mutate_label, is_free, compiler='/home/jwzeng/compilers/gcc/gcc-13.1.0/bin/gcc', proc_num=-1):
        ...
        if ret_code == 0:
            ...
            # 如果此次变异相似度大于之前所有变异的相似度平均值，则奖励+1
...
if __name__ == '__main__':
    t = 101            # 总变异次数
    tj = 6         # tj表示直到第t次迭代为止选择第j个增变器/限制选项的总数
    xji_sum = 0     # xji指第i次迭代中第j个增变器/限制选项的奖励；这里是tj次总和
    # UCB-1
    res = float(xji_sum / tj) + math.sqrt((2 * np.log(t - 1)) / tj)
    print(res)
