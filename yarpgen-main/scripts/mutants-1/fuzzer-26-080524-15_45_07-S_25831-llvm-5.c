#include "/home/yixuan/csmith/runtime/csmith.h"

// insert-while-snippet -> insert-if-snippet -> insert-switch-snippet -> insert-while-snippet -> insert-while-snippet -> insert-switch-snippet

static short a = 7;
int b = 1;
int c;
int d;
int e;
int f;
void fn1(short *p1)
{
  int * D4WzKBc2 = &b;
  unsigned char rDA6olQC = (unsigned char)255;
  int ir68Dp4V = 2147483647;
  int V0kjeZFP = 252913411;
  int PuM0_8zO = 1622132336;
  int zjEtXKv0 = 839440773;
  int x84UVdJD = 673660045;
  int nz2YvhSy = 2147483647;
  int JSoBMcZ4 = -774503690;
  int E7dsOjCW = -2147483648;
  int IsjWMyD8 = 2147483647;
  int a4zhopDU = 801471337;
  int bBhxz5Yv = -2147483648;
  int ykMWQC32 = 1665688297;
  int QEXNt_xv = -2147483648;
  int EbIm2863 = -583306207;
  b = (4 % a) & (a / b);
  d = f && (1 / d);
  a = f | (a ^ 1);
  if (a || a)
  {
    printf("l_282=%lld\n", (long long) f);
  }
  printf("g_9=%lld\n", (long long) a);
  d = a;
  *p1 = e;
  while (1)
  {
    if (a4zhopDU > bBhxz5Yv)
      break;
    while (1)
    {
      int h = QEXNt_xv % ykMWQC32;
      h = ykMWQC32;
      ykMWQC32 = IsjWMyD8;
      if (QEXNt_xv)
        ;
      ykMWQC32 = h;
      L:
      E7dsOjCW = ykMWQC32;

      ykMWQC32 = ~ykMWQC32;
      a4zhopDU = (QEXNt_xv ^ E7dsOjCW) ^ (~bBhxz5Yv);
      if (!ykMWQC32)
        break;
      ykMWQC32 = ~EbIm2863;
    }

  }

  transparent_crc(EbIm2863, "EbIm2863", 1);
  transparent_crc(QEXNt_xv, "QEXNt_xv", 1);
  transparent_crc(ykMWQC32, "ykMWQC32", 1);
  transparent_crc(bBhxz5Yv, "bBhxz5Yv", 1);
  transparent_crc(a4zhopDU, "a4zhopDU", 1);
  transparent_crc(IsjWMyD8, "IsjWMyD8", 1);
  transparent_crc(E7dsOjCW, "E7dsOjCW", 1);
  transparent_crc(JSoBMcZ4, "JSoBMcZ4", 1);
  transparent_crc(nz2YvhSy, "nz2YvhSy", 1);
  transparent_crc(x84UVdJD, "x84UVdJD", 1);
  transparent_crc(zjEtXKv0, "zjEtXKv0", 1);
  transparent_crc(PuM0_8zO, "PuM0_8zO", 1);
  transparent_crc(V0kjeZFP, "V0kjeZFP", 1);
  transparent_crc(ir68Dp4V, "ir68Dp4V", 1);
  transparent_crc(rDA6olQC, "rDA6olQC", 1);
  platform_main_end(crc32_context ^ 0xFFFFFFFFUL, 0);
  if (a && (f < 4))
  {
    while (1)
    {
      int f = 2;
      int g = 1;
      int h = 126;
      int *i = &h;
      (&V0kjeZFP) == i;
      L1:
      L2:
      if (!h)
        goto L1;


      if (!f)
      {
        zjEtXKv0 = (g << h) || (zjEtXKv0 << 3);
        goto L2;
        switch (rDA6olQC)
        {
          case 0x95:
            *D4WzKBc2 = rDA6olQC - 0x91;
            break;

          case 0x80:

          case 0x81:

          case 0x82:
            *D4WzKBc2 = rDA6olQC - 0x80;
            break;

          default:
            *D4WzKBc2 = 0;
            break;

        }

      }
      PuM0_8zO = ir68Dp4V;
      f = (h = (ir68Dp4V = g));
      g = PuM0_8zO;
      if (PuM0_8zO)
        break;
      goto L2;
    }

    if (x84UVdJD)
    {
      JSoBMcZ4 = nz2YvhSy;
      __builtin_abort();
    }
    c--;
  }
}

int main()
{
  platform_main_begin();
  crc32_gentab();
  long F9Zb64M7 = -9223372036854775807LL;
  int wNVY5lai = -1071225760;
  long crGR9TgM = -1164003582564288457LL;
  int * UvTBK3dQ = &b;
  unsigned char SLH94UmZ = (unsigned char)118;
  fn1(&a);
  printf("%d\n", c);
  switch (SLH94UmZ)
  {
    while (F9Zb64M7--)
      *((char *) (crGR9TgM++)) = wNVY5lai;

    case 0x95:
      *UvTBK3dQ = 0x01;
      break;

    case 0x80:

    case 0x81:

    case 0x82:
      *UvTBK3dQ = 0x00;
      break;

    default:
      *UvTBK3dQ = 0x05;
      break;

  }

  transparent_crc(SLH94UmZ, "SLH94UmZ", 1);
  transparent_crc(crGR9TgM, "crGR9TgM", 1);
  transparent_crc(wNVY5lai, "wNVY5lai", 1);
  transparent_crc(F9Zb64M7, "F9Zb64M7", 1);
  platform_main_end(crc32_context ^ 0xFFFFFFFFUL, 0);
  return 0;
}

