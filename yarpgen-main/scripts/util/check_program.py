...
# -*- encoding: utf-8 -*-
'''
@file        : check_program.py
@description : 
@time        : 2023/05/29 19:22:25
@author      : jwzeng
'''
...
class Run_Sanitizer():
    clang_compile = '/usr/lib/llvm-18/bin/clang'
    gcc_compile = '/home/yixuan/compiler/gcc-13.2.0/build/bin/gcc'
...
def check_program_by_sanitizer(clang_compiler, sanitizer, program_file_path, tmp_dir, proc_num=-1, seed=-1, 
    ...
    if ret_code != 0 or is_time_expired or not os.path.exists(os.path.join(tmp_dir, "a.out")):
        ...
        # global KNOWN_BUILD_FAILS
        # for reg_expr, tag in KNOWN_BUILD_FAILS.items():
        #     if re.search(reg_expr, str(err_output, "utf-8")):
        #         file_list = program_file_path
        #         log_file_path = os.path.join(tmp_dir, 'log.txt')
        #         build_compile_fail_log(log_file_path, seed, tag, mutation_operator_label, ' '.join(check_cmd), err_output)
        #         file_list.append(log_file_path)
        #         save_test(lock, file_list, compiler_name='sanitizer_compile', fail_type=tag, test_name=str(seed))
        #         break
        ...
        if ret_code != 0 or is_time_expired or err_output.decode().strip() != '':
            ...
            # file_list = program_file_path
            # log_file_path = os.path.join(tmp_dir, 'log.txt')
            # build_compile_fail_log(log_file_path, seed, sanitizer, mutation_operator_label, ' '.join(check_cmd), err_output)
            # file_list.append(log_file_path)
            # save_test(lock, file_list, compiler_name='sanitizer_run', fail_type=sanitizer, test_name=str(seed))
            return False
        return True
...
'''
COMPILER2REAL = {
    'clang': '/usr/local/bin/clang-18 -Werror=uninitialized',
    'clang-c': '/usr/local/bin/clang-18 -Werror=uninitialized -c',
    'gcc': '/home/yixuan/compiler-older/gcc-13.2.0/build/bin/gcc  -Werror=uninitialized',
    'gcc-c': '/home/yixuan/compiler-older/gcc-13.2.0/build/bin/gcc  -Werror=uninitialized -c',
    # 'tcc': 'tcc',
    # 'tcc-c': 'tcc -c',
    'icx': 'icx -fPIC -mcmodel=large -w -fopenmp-simd -mllvm -vec-threshold=0',
    'icx-c': 'icx -fPIC -mcmodel=large -w -fopenmp-simd -mllvm -vec-threshold=0 -c',
...
def check_compiler_ICE(compiler, program_file_path, tmp_dir, proc_num=-1, seed=-1,
    ...
    # done: 这里部返回True or False，因为这里只是检查编译器是否会崩溃
...
def check_program_by_all_sanitizers_with_one_single_threaded(clang_compiler, program_file_path, tmp_dir,
    ...
    # ! 这下面用于单线程测试
    # with open(os.path.join(tmp_dir, 'count.txt'), 'w') as f:
    #     for k, v in mutation2count.items():
    #         f.write(f'{k}: {v}\n')
    # sanitizer_log = os.path.join(tmp_dir, 'sanitizer-' + str(seed) + '.log')
    # with open(sanitizer_log, 'a+') as f:
    #     f.write('****************************************************\n')
